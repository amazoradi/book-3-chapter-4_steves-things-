...
githubData.forEach((event) => {
  let i = githubData.indexOf(event)
  eventArr.push(githubData[i].type)
  if (eventArr[i] === "PushEvent") {
    pushArr.push(eventArr[i]);
    numPush = pushArr.length;
  }
  else if (eventArr[i] === "PullRequestEvent") {
  ...
  else if (eventArr[i] === "CreateEvent") {
    createArr.push(eventArr[i]);
    numCreate = createArr.length;
  }
  else if (eventArr[i] === "IssueCommentEvent") {
    issueArr.push(eventArr[i]);
    numIssue = issueArr.length;
  }
})
...
githubData.forEach((event) => {
  if (event.payload.hasOwnProperty("pull_request")){
   langs.push(event.payload.pull_request.head.repo.language)
  }
});
...
langs.forEach((item) => {
  langsCount[item] = (langsCount[item] || 0) + 1;
})
